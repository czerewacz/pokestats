...
import ai.akun.pokeanalyzer.data.toPokeAverage
import ai.akun.pokeanalyzer.domain.model.PokemonAverage
import kotlinx.coroutines.flow.*
import kotlin.coroutines.CoroutineContext

class GetPokeAveragesUseCase(
        ...
        return getPokemonUrlsUseCase(
            ...
            when (result) {
                is GetPokemonStatsUseCase.Result.Success -> {
                    Result.Success(result.pokemons.toPokeAverage())
                }
    ...
    sealed class Result {
        class Success(val pokemonAverage: PokemonAverage) : Result()
